...
public class SyInstrumentView extends View {
    ...
    public SyInstrumentView(Context context) {
        this(context, null);
    }

    public SyInstrumentView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SyInstrumentView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        canvas.drawText("当前进度："+progress+"%",);
        ...
        canvas.drawText(progress + "元", centerPoint[0], baseLine1, mPaint);


//        float baseLine2 = outCircleRadius / 20f * 11 - 3 * (fontMetrics.bottom + fontMetrics.top) + centerPoint[1];
//        canvas.drawText(progress/10 + "时", centerPoint[0], baseLine2, mPaint);
    ...
    private void drawPointer(Canvas canvas) {
        ...
        int angle = (int) ((allAngle) / (1000 * 1f) * progress) + startAngle;
        //指针的定点坐标
     ...
     * 根据进度画进度条
     *
     * @param progress 最大进度为1000.最小为0
     */
    private void drawProgress(double progress, Canvas canvas) {
        double ratio = progress / 1000f;
